...
- [ ] **Conflict self-healing left on** — no `rebase-strategy: disabled`. The default `auto` lets Dependabot rebase open PRs and regenerate the lockfile when the base moves, self-healing the lockfile conflicts that otherwise pile up; disabling it forfeits that. Flag an explicit `rebase-strategy: disabled` unless the repo has a stated reason. (Auto only heals what a lockfile regen fixes; manifest-level conflicts still hang on `CONFLICTING` — the sibling **`automerge-setup`** conflict-recovery companion covers that residual gap.)
