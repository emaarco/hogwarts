...
- [ ] **Conflict self-healing on and explicit** — never `rebase-strategy: disabled` (it forfeits Dependabot rebasing open PRs and regenerating the lockfile when the base moves, self-healing the lockfile conflicts that otherwise pile up); flag an explicit `disabled` unless the repo has a stated reason. Prefer an **explicit `rebase-strategy: auto`** on every standard block (Phase 5 writes it) as a self-documenting, drift-proof signal — its absence isn't a defect since `auto` is the default, but note it. (Auto only heals what a lockfile regen fixes; manifest-level conflicts still hang on `CONFLICTING` — the sibling **`automerge-setup`** conflict-recovery companion covers that residual gap.)
...
- Every mode keeps: `labels: ['dependencies']`, `commit-message` prefix `chore` + `include: scope`, `cooldown`, an explicit `rebase-strategy: auto` on every standard block, and an `applies-to: security-updates` group per ecosystem.
- **Write `rebase-strategy: auto` explicitly** on every standard update block — don't rely on it being the default. This is a deliberate exception to *don't restate defaults* below: conflict self-healing is load-bearing (Dependabot rebases open PRs and regenerates the lockfile when the base moves), so state it to document the intent and guard against a future `disabled` slipping in. **Never write `rebase-strategy: disabled`.** Member blocks joining a multi-ecosystem group can't carry it (same restriction as `schedule`/`cooldown`) — they inherit Dependabot's default `auto` implicitly; note that rather than trying to set it there. The primary conflict-*avoidance* lever, though, is **grouping** (minor/patch bundled into one PR per ecosystem): fewer, wider PRs mean fewer bases to keep rebased and far fewer chances to collide on the same manifest/lockfile. Auto-rebase can't fix manifest-level conflicts — for the PR that still hangs on `CONFLICTING`, the sibling **`automerge-setup`** ships a conflict-recovery companion.
