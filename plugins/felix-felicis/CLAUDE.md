...
- **optimize-github-actions** — CI run-efficiency audit & fix: duplicate PR runs from `push` + `pull_request` double-triggers (invisible to ref-based concurrency groups), matrix job explosion vs. a job budget, missing/miswired `concurrency` (never cancel-in-progress on deploy/publish), noisy `pull_request` types & draft handling, and the `paths:`-on-required-check pending-forever trap (rollup-gate conversion, sequenced with `branch-ruleset-setup`). Live evidence via `gh` with static-only fallback; all trigger changes gated on merge-gate verification (R6, incl. `merge_group`), with tag-only push triggers and `pull_request_target` explicitly out of rewrite scope.
...
- **automerge-setup** — Single source of truth for GitHub PR auto-merge: detects/audits/optimizes or creates the setup across three strategies (Dependabot via `fetch-metadata` patch/minor — the default; Renovate via native `automerge`+`platformAutomerge`; generic bot via actor+label). Gates on the two safety preconditions — native `allow_auto_merge` repo setting and a required-status-check ruleset — grades the CI signal (what a green check proves: lint vs build vs unit vs integration/E2E, weighed against what the repo ships) and caps the scope recommendation to that grade, and catches drift (e.g. a single-shot `gh pr merge --auto` missing the UNSTABLE-race retry loop); majors always stay manual. Owns the `reference/*-automerge-workflow.yml` templates that `dependabot-setup` used to carry, and delegates the neighbours (`branch-ruleset-setup`, `pin-github-actions`, `dependabot-setup`, runtime arming to `dependency-update-shepherd`).
...
- **guardrails-setup** — Introduces and maintains machine-checkable guardrails (fitness functions) so AI agents can work safely in a repo — grounded in Uncle Bob's "measure, don't review agent code" position. Three modes: `audit` (gap report against the repo's tier, read-only), `setup` (Phase 0 baseline → plan challenged by three persona subagents → per-phase approval → one PR per phase), `phase <n>` (implement one phase from the committed plan). Right-sized via a T1–T4 tier matrix; all ratchet state in one `guardrails/ratchets.json` diffed by a self-protecting Phase 2b gate (raise-only thresholds, shrink-only debt/exclusion lists, guardrail-path edits gated); exactly one exception path (human-applied PR label, CI-checked, CODEOWNERS-backed via `branch-ruleset-setup`); every gate negative-tested with the red output in the PR. Playbook in `references/` covers phases 0–5, starting thresholds, tool map per language, and pitfalls.
...
- **dependency-update-shepherd** — Shepherds open dependency-update branches/MRs (Renovate/Dependabot/manual) from red/stale to merged, one at a time: bot-aware rebase first (bot's own rebase preferred; a manual push is a recorded takeover after which bot rebase is forbidden), then first-causal CI failure → classify (one flaky rerun per MR) → minimal local fix → verify → `--force-with-lease` push → watch the own push's pipeline (GitLab: MR pipelines via `glab ci get --merge-request`). Safety: candidates authenticated by verified bot identity, conservative lockfile regeneration diffed against the bot's original, 3-iteration cap, fresh merge gate; merging/MR-writes need Phase-0 opt-in, agent-authored code fixes a per-diff confirmation, major-equivalents (majors, `0.x` minors, SHA/digest pins) never auto-merged unless named. Reports per MR incl. everything not verified.
